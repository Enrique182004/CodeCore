...
class WorkshopArchiver {
  ...
  shouldArchive(workshop) {
    ...
    const now = new Date();
    const workshopEndTime = this.parseWorkshopDateTime(
      workshop.date,
      workshop.time,
    );
    ...
    if (!workshopEndTime) {
      console.error(
        "Could not determine end time for workshop:",
        workshop.title,
      );
      return false;
    }
  ...
  archiveWorkshop(workshop) {
    ...
    if (
      workshop.formLink.includes("forms.gle") ||
      workshop.formLink.includes("forms.google.com")
    ) {
      console.log(
        `Workshop "${workshop.title}" still has registration form link - consider updating to materials link`,
      );
    }
  ...
  saveArchiveAction(workshop) {
    try {
      let archivedWorkshops = JSON.parse(
        localStorage.getItem("archivedWorkshops") || "[]",
      );
      ...
      if (!archivedWorkshops.find((w) => w.id === workshop.id)) {
        ...
        localStorage.setItem(
          "archivedWorkshops",
          JSON.stringify(archivedWorkshops),
        );
      }
    } catch (e) {
