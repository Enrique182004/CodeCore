...
function updateStats() {
  ...
  const avgScore =
    filteredParticipants.length > 0
      ? Math.round(
          filteredParticipants.reduce((sum, p) => sum + p.score, 0) /
            filteredParticipants.length,
        )
      : 0;
...
function applyFilters() {
  ...
  if (currentFilter === "underclassmen") {
    filtered = filtered.filter(
      (p) => p.classStanding === "freshman" || p.classStanding === "sophomore",
    );
  } else if (currentFilter === "upperclassmen") {
    filtered = filtered.filter(
      (p) => p.classStanding === "junior" || p.classStanding === "senior",
    );
...
function renderLeaderboard() {
  ...
  const showInPodium =
    currentSort === "score" && filteredParticipants.length >= 3;
  ...
  const scoreSortedList = [...filteredParticipants].sort(
    (a, b) => b.score - a.score,
  );

  tbody.innerHTML = displayParticipants
    .map((participant, index) => {
      // ALWAYS find the participant's rank based on score within the current filter
      const rank =
        scoreSortedList.findIndex(
          (p) => p.name === participant.name && p.email === participant.email,
        ) + 1;
